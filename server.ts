...
if (apiKey && apiKey !== "MY_GEMINI_API_KEY" && apiKey.trim() !== "") {
  ...
  console.log("Gemini API key not configured or contains placeholder. Falling back to structured templates.");
}

// REST route to generate clinical observation sheets
app.post("/api/generate-report", async (req, res) => {
  const { patient, thyroid, foot } = req.body;

  if (!patient || !thyroid || !foot) {
    return res.status(400).json({ error: "Missing required clinical examination data" });
  }

  // Construct a very dense clinical prompt
...
`;
  ...
  try {
    if (ai) {
      ...
      if (generatedText) {
        return res.json({ report: generatedText, isBackup: false, model: successfulModel });
      } else {
        throw lastError || new Error("All fallback models failed to generate content");
      }
    } else {
      // Return beautiful generated text template when API goes offline
      console.log("Using backup template engine...");
      const backupReport = generateBackupLocalReport(patient, thyroid, foot);
      return res.json({ report: backupReport, isBackup: true });
    }
  } catch (error: any) {
    console.error("Gemini requests failed. Using backup template engine.", error);
    const backupReport = generateBackupLocalReport(patient, thyroid, foot);
    return res.json({ report: backupReport, isBackup: true, error: error.message });
  }
});

// Advanced local medical template generator in French
function generateBackupLocalReport(patient: any, thyroid: any, foot: any) {
  ...
  let isNeuropathic = false;
  let isArteriopathic = false;

  if (neuroSymptoms.length > 0 || foot.sensibility.tactile === "Altérée" || foot.sensibility.algesic === "Altérée") {
    isNeuropathic = true;
  }
  if (foot.art.claudication || foot.palpation.pedalPulse === "Absent" || foot.palpation.pedalPulse === "Diminué" || foot.palpation.tibialPulse === "Absent") {
    isArteriopathic = true;
  }
...
// Vite development / production handler
